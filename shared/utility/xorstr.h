...
namespace jm {
    ...
    namespace detail {
        ...
        XORSTR_FORCEINLINE constexpr std::uint64_t load_xored_str8(
            std::uint64_t key,
            std::size_t idx,
            const CharT *str
        ) noexcept {
        ...
        XORSTR_FORCEINLINE void crypt() noexcept {
...
#else
            alignas(alignment)
                std::uint64_t keys[] { ::jm::detail::load_from_reg(Keys)... };
#endif
            ...
            if constexpr ( sizeof(_storage) % 32 != 0 ) {
                _mm_store_si128(
                    ...
                    _mm_xor_si128(
                        _mm_load_si128(reinterpret_cast<const __m128i *>(_storage + sizeof...(Keys) - 2)),
