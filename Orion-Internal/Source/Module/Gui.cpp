		...
		if (Menu::Body body{}) {
			if (Menu::Body::Top top{}) {
				...
				if (Menu::Tab tab{ Orion::Fnv<"Configs">::value }) {
					...
					Menu::Body::Top::Combo<"Ally\0Enemy\0">(config->getData().players.current);
				}
			}
			if (Menu::Body::Content content{}) {
				if (Menu::Body::Content::Main main{}) {
							...
							if (Menu::Body::Content::Main::Panel::Table table{}) {
								if (Menu::Body::Content::Main::Panel::Table::Group<"General", true> group{}) {
									if (Menu::Body::Content::Main::Panel::Table::Widget widget{}) {
										auto&& data = config->getData().players;
										switch (data.current) {
										case 0:
										{
											widget.Toggle<"Box">(data.ally.box.enable, data.ally.box.color, m_colorReference, &m_popupAlpha);
											widget.Toggle<"Name">(data.ally.name);
											widget.Toggle<"Skeleton">(data.ally.skeleton);
											widget.Toggle<"Health Bar">(data.ally.healthbar);
											widget.Toggle<"Weapon Icon">(data.ally.weapon.icon);
										}
										break;
										case 1:
										{
											widget.Toggle<"Box">(data.enemy.box.enable, data.enemy.box.color, m_colorReference, &m_popupAlpha);
											widget.Toggle<"Name">(data.enemy.name);
											widget.Toggle<"Skeleton">(data.enemy.skeleton);
											widget.Toggle<"Health Bar">(data.enemy.healthbar);
											widget.Toggle<"Weapon Icon">(data.enemy.weapon.icon);
										}
										break;
										}
									}
					...
					else if (Menu::Tab tab{ Orion::Fnv<"View">::value }) {
						if (Menu::Body::Content::Main::Panel panel{}) {
							if (Menu::Body::Content::Main::Panel::Table table{}) {
								if (Menu::Body::Content::Main::Panel::Table::Group<"Crosshair", true> group{}) {
									if (Menu::Body::Content::Main::Panel::Table::Widget widget{}) {
										auto&& data = config->getData().view.crosshair;
										widget.Toggle<"Enable">(data.enable, data.color, m_colorReference, &m_popupAlpha);
										widget.Slider<"Size", "%.1f", 1.f, 8.f>(data.size);
									}
								}
								if (Menu::Body::Content::Main::Panel::Table::Group<"Misc", true> group{}) {
									if (Menu::Body::Content::Main::Panel::Table::Widget widget{}) {
										auto&& data = config->getData().view.misc;
										widget.Slider<"Aspect Ratio", "%.2f", 0.f, 5.f>(data.aspect_ratio);
									}
								}
							}
						}
						...
						if (Menu::Body::Content::Main::Panel panel{}) {
							if (Menu::Body::Content::Main::Panel::Table table{}) {
								if (Menu::Body::Content::Main::Panel::Table::Group<"Movement", true> group{}) {
									if (Menu::Body::Content::Main::Panel::Table::Widget widget{}) {
										auto&& data = config->getData().main.movement;
										widget.Toggle<"Auto Jump">(data.auto_jump);
									}
								}
							}
						}
