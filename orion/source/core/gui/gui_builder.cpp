...
Font::Font(ImFont& font, const float scale) noexcept :
    font {font},
    scale {font.Scale} {
    font.Scale = scale;
    ImGui::PushFont(&font);
...
auto Header::Nav::watermark() noexcept -> void {
    const auto& style = ImGui::GetStyle();
    ImGui::BeginChild(
        Fnv1a<"##Header::Nav::watermark">::value,
        ImVec2 {0.0f, 62.0f},
        false,
        ImGuiWindowFlags_::ImGuiWindowFlags_NoBackground
            | ImGuiWindowFlags_::ImGuiWindowFlags_NoDecoration
...
auto Header::Nav::profile() noexcept -> void {
    ...
    ImGui::BeginChild(
        Fnv1a<"##Header::Nav::profile">::value,
        ImVec2 {0.0f, 0.0f},
        false,
        ImGuiWindowFlags_::ImGuiWindowFlags_NoBackground
            | ImGuiWindowFlags_::ImGuiWindowFlags_NoDecoration
...
auto Body::Head::create() const noexcept -> void {
    const auto label =
        ...
        + String<"    ">().c_str() + String<"Create">().c_str();
    const auto alpha = ImGui::GetStyle().Alpha * 1.0f;
...
auto Body::Head::combo(const char* const items, int& value) noexcept -> bool {
    ...
    const auto result =
        ImGui::Combo(String<"##Body::Head::combo">().c_str(), &value, items);
    ImGui::PopStyleVar(6);
...
Body::Content::Panel::Panel(const std::uint32_t hash) noexcept {
    ...
    if (const auto tab = gui.get_tabs().find(hash);
        ...
        ImGui::BeginChild(
            Fnv1a<"##Body::Content::Panel">::value,
            ImVec2 {},
            true,
            ImGuiWindowFlags_::ImGuiWindowFlags_NoBackground
...
auto Body::Content::Panel::Config::draw(
    ...
    if (!Fnv1a<"Default">::eq(name.data())) {
        ...
        String<"##Menu::Body::Content::Main::Panel::Config::Rename">
            window_rename;
        ImGui::SetCursorPos(
        ...
        ImGui::SetNextWindowSize(
            ImVec2 {256.0f, 35.0f}
                * utilities::math::easing::in_out_quart(popup_alpha),
...
auto Body::Content::Panel::Config::filter_config_name(
...
Body::Content::Panel::Table::Table() noexcept {
    ...
    Table::resume =
        ImGui::BeginTable(String<"##Body::Content::Panel::Table">().c_str(), 2);
    if (Table::resume) {
        ImGui::TableNextRow();
    }
}
...
auto Body::Content::Panel::Table::begin_group(
    ...
    ImGui::GetWindowDrawList()->AddRectFilled(
        pos,
        pos + ImVec2 {ImGui::GetContentRegionAvail().x + line_width_offset * 2.0f, line_height},
