...
struct Header::Nav::Items final
{
  ...
  private:
    static auto button(const char* const str, const char* const icon, const std::uint32_t hash) noexcept -> void;

    auto text(const char* const str) noexcept -> void;

  public:
    template <stb::fixed_string _Str> constexpr auto button(const char* const icon) const noexcept
    {
        return Items::button(String<_Str>(), icon, Fnv1a<_Str>::value);
    }

    template <stb::fixed_string _Str> constexpr auto text() noexcept
    {
...
struct Body::Head final
{
  ...
  private:
    NODISCARD static auto combo(const char* const items, int& value) noexcept -> bool;

  public:
    template <stb::fixed_string _Str> NODISCARD constexpr auto combo(int& value) const noexcept
    {
...
struct Body::Content::Panel::Config final
{
  ...
  private:
    static auto filter_config_name(ImGuiInputTextCallbackData* const data) noexcept -> int;
    ...
    auto draw(const std::string_view name, const std::string_view time, const bool active,
...
struct Body::Content::Panel::Table final
{
    ...
    template <stb::fixed_string _Str, bool _NextColumn> struct Group;
    struct Widget;

    explicit Table() noexcept;
    ~Table() noexcept;

    static auto begin_group(const char* const name, const std::uint32_t hash, const bool next_column) noexcept -> void;
...
template <stb::fixed_string _Str, bool _NextColumn> struct Body::Content::Panel::Table::Group final
{
    NON_COPYABLE(Group)
    NON_MOVEABLE(Group)
...
struct Body::Content::Panel::Table::Widget final
{
  ...
  public:
    ...
    constexpr void toggle(bool& value, float color[4], float colorReference[4], float* popupAlpha) noexcept
    {
        ...
        const Gui::PushFont font{orion.get_gui().get_fonts().factory, (fontHeight / 15.f)};
        float ratio = 0;
        ...
        if (ImGui::TableSetColumnIndex(1))
        {
            const auto pos = ImGui::GetCursorPos() +
                             ImVec2{ImGui::GetContentRegionAvail().x - ImGui::GetFrameHeight() * toggleWidthMult,
                                    (ImGui::GetFrameHeight() - ImGui::GetFrameHeight() * .8f) * .5f};
            ImGui::SetCursorPos(pos);
            ratio = ImGui::ButtonToggle(name.c_str(), value, toggleWidthMult);
            ...
            if constexpr (tip[0] != '\0')
            {
                    ...
                    ImGui::GetCurrentContext()->HoveredIdTimer > 0.5f)
                {
                    ...
                    const auto& popup_bg = colors[ImGuiCol_::ImGuiCol_PopupBg];
                    const auto& text_col = colors[ImGuiCol_::ImGuiCol_Text];
                    const auto alpha = utilities::math::easing::out_expo(
                        std::clamp(ImGui::GetCurrentContext()->HoveredIdTimer - 0.5f, 0.0f, 1.0f));
                    const StyleColor color[] = {
                        {ImGuiCol_::ImGuiCol_PopupBg,
                         ImVec4{popup_bg.x, popup_bg.y, popup_bg.z, std::clamp(alpha, 0.0f, popup_bg.w)}},
                        {ImGuiCol_::ImGuiCol_Text,
                         ImVec4{text_col.x, text_col.y, text_col.z, std::clamp(alpha, 0.0f, text_col.w)}},
                    };
            ...
            if (color)
            {
                ...
                const auto colorLabel{std::string{colorIcon} + name.c_str()};

                const StyleVar styleVar[]{{ImGuiStyleVar_::ImGuiStyleVar_FrameRounding, 3},
                ...
                if (ImGui::Button(std::string{std::string{colorIcon} + String<"##">().c_str() + name.c_str()}.c_str()))
                {
                ...
                const StyleVar styleVar2[]{{ImGuiStyleVar_::ImGuiStyleVar_Alpha, *popupAlpha}};
                ImGui::SetNextWindowSize(ImVec2{308, 256} * utilities::math::easing::in_out_quart(*popupAlpha),
                ...
                if (ImGui::BeginPopup(colorLabel.c_str(), ImGuiWindowFlags_::ImGuiWindowFlags_NoScrollbar))
                {
                    ImGui::ColorPicker4(colorLabel.c_str(), color,
                                        ImGuiColorEditFlags_::ImGuiColorEditFlags_NoLabel |
                                            ...
                                            ImGuiColorEditFlags_::ImGuiColorEditFlags_AlphaPreview,
                                        colorReference);
                    ImGui::EndPopup();
                    *popupAlpha = std::clamp(*popupAlpha + ImGui::GetIO().DeltaTime, 0.f, 1.f);
                }
            }

            if (ImGui::TableSetColumnIndex(0))
            {
                ImGui::SetCursorPos(ImGui::GetCursorPos() + ImVec2{0, (ImGui::GetFrameHeight() - fontHeight) * .5f -
                                                                          (color ? textPositionVerticalOffset : 0)});
                ImGui::TextColored(
                    ImLerp(ImVec4{.564705f, .615686f, .647058f, 1}, ImVec4{.90196f, .90196f, .90196f, 1}, ratio), name);
            }
        }
    }

    template <stb::fixed_string str, stb::fixed_string tip = ""> constexpr auto toggle(bool& value) noexcept
    {
        return toggle<str, tip>(value, nullptr, nullptr, nullptr);
    }

    template <stb::fixed_string str, stb::fixed_string items> auto combo(int& value) noexcept
    {
        String<str> name;
        String<items> item;
        ...
        if (ImGui::TableSetColumnIndex(0))
        {
            ImGui::SetCursorPos(ImGui::GetCursorPos() +
                                ImVec2{0, (ImGui::GetFrameHeight() - fontHeight - framePadding * .5f) * .5f -
                                              textPositionVerticalOffset});
            ImGui::TextColored(ImVec4{.564705f, .615686f, .647058f, 1}, name);
        }
    }

    template <stb::fixed_string str, stb::fixed_string items> auto multi_combo(bool value[]) noexcept
    {
        constexpr auto fontHeight{14.00f};
        constexpr auto framePadding{4.00f};
        constexpr auto textPositionVerticalOffset{2.00f};
        ...
        if (ImGui::TableSetColumnIndex(1))
        {
            ...
            if (ImGui::BeginCombo(std::string{std::string{String<"##">().c_str()} + name.c_str()}.c_str(),
                                  preview.c_str()))
            {
                auto p{item.c_str()};
                while (*p)
                ...
                {
                    ImGui::Selectable(list[i].c_str(), &value[i],
            ...
            {
                i == selected.size() - 1 ? preview += selected[i] : preview += selected[i] + String<", ">().c_str();
            }
        }
    ...
    template <stb::fixed_string str, stb::fixed_string fmt, float min, float max> auto slider(float& value) noexcept
    {
        constexpr auto fontHeight{14.00f};
        ...
        if (ImGui::TableSetColumnIndex(1))
        {
            ...
            {
                const Gui::PushFont font{orion.get_gui().get_fonts().factory, (fontHeight - 1) / 15.f};
                const StyleColor styleColor2[]{
                    {ImGuiCol_::ImGuiCol_FrameBg, ImVec4{.0470588f, .0470588f, .0470588f, 1}},
                };
                ImGui::SetCursorPos(pos +
                                    ImVec2{ImGui::GetContentRegionAvail().x - inputTextWidth - style.CellPadding.x, 0});
                ImGui::SetNextItemWidth(inputTextWidth);
                if (ImGui::InputFloat(std::string{std::string{String<"##">().c_str()} + name.c_str()}.c_str(), &value,
                                      0.f, 0.f, format, ImGuiInputTextFlags_::ImGuiInputTextFlags_AutoSelectAll))
                {
            ...
            ImGui::SliderFloat5(std::string{std::string{String<"###">().c_str()} + name.c_str()}.c_str(), value, min,
                                max, ImGui::GetFrameHeight() * .35f,
                                IM_COL32(32, 32, 32, static_cast<int>(style.Alpha * 255)), format,
