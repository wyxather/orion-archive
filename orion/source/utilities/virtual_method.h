#pragma once
    ...
    #define DEFINE_VIRTUAL_METHOD(index, name, type, params, args) \
        NODISCARD constexpr auto name params const noexcept -> type { \
            return ::orion::utilities::VirtualMethod:: \
                call<static_cast<std::size_t>(index), type> \
                    args; \
        }
#endif
...
namespace orion::utilities {
    ...
    public:
        ...
        call(ClassType self, Args... args) noexcept -> ReturnType {
            using FunctionType =
                ReturnType(__thiscall * * const*)(ClassType, Args...);
            const auto virtual_method_table =
                *reinterpret_cast<FunctionType>(self);
