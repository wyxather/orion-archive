...
namespace orion::utilities {
    ...
    template<typename Data, typename Key = std::uint32_t>
    class HashMap {
        std::vector<std::pair<Key, Data>> data;

        template<typename... Args>
        constexpr auto emplace(Key key, Args&&... args) noexcept -> void {
            data.emplace_back(
    ...
    public:
        ...
        NODISCARD constexpr auto find(Key key) noexcept -> Data* {
            if (const auto it = std::ranges::lower_bound(
                    ...
                    key,
                    {},
                    &decltype(data)::value_type::first
                );
        ...
        template<typename... Args>
        constexpr auto insert(Key key, Args&&... args) noexcept -> void {
            if (find(key) != nullptr) {
                return;
            }
            data.emplace_back(
