...
struct Vector3 final
{
    ...
    constexpr explicit Vector3(float x, float y = 0.0f, float z = 0.0f) noexcept : x(x), y(y), z(z)
    {
    }

    NODISCARD static auto from_angle(const Vector3& angle) noexcept -> auto
    {
        return Vector3{std::cos(utilities::math::deg_to_rad(angle.x)) * std::cos(utilities::math::deg_to_rad(angle.y)),
                       std::cos(utilities::math::deg_to_rad(angle.x)) * std::sin(utilities::math::deg_to_rad(angle.y)),
                       -std::sin(utilities::math::deg_to_rad(angle.x))};
    }

    NODISCARD friend constexpr auto operator+(const Vector3& a, const Vector3& b) noexcept -> auto
    {
    ...
    NODISCARD friend constexpr auto operator+(float f, const Vector3& v) noexcept -> auto
    {
    ...
    NODISCARD constexpr auto operator==(const Vector3& o) const noexcept -> bool = default;
    ...
    NODISCARD constexpr auto dot_product(const Vector3& o) const noexcept -> auto
    {
