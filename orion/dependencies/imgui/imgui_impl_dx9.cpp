...
struct ImGui_ImplDX9_Data
{
    ...
    ComPtr<IDirect3DVertexShader9>      vertex_shader;
    ComPtr<IDirect3DVertexDeclaration9> vertex_declaration;
...
static void ImGui_ImplDX9_SetupRenderState(ImDrawData* draw_data)
{
    ...
    bd->pd3dDevice->SetVertexShader(bd->vertex_shader.Get());
...
void ImGui_ImplDX9_RenderDrawData(ImDrawData* draw_data)
{
    ...
    bd->pd3dDevice->SetVertexDeclaration(bd->vertex_declaration.Get());
...
bool ImGui_ImplDX9_CreateDeviceObjects()
{
    ...
    if (bd->vertex_declaration.Get() == nullptr) {
        constexpr std::array<D3DVERTEXELEMENT9, 4> elements {
            D3DVERTEXELEMENT9 {
                0,
                IM_OFFSETOF(CUSTOMVERTEX, pos),
                D3DDECLTYPE_FLOAT2,
            ...
            D3DVERTEXELEMENT9 {
                0,
                IM_OFFSETOF(CUSTOMVERTEX, col),
                D3DDECLTYPE_D3DCOLOR,
                D3DDECLMETHOD_DEFAULT,
                D3DDECLUSAGE_COLOR,
                0},
            D3DDECL_END()};
        bd->pd3dDevice->CreateVertexDeclaration(
            elements.data(),
            bd->vertex_declaration.GetAddressOf());
    }
    if (bd->vertex_shader.Get() == nullptr) {
        bd->pd3dDevice->CreateVertexShader(
            reinterpret_cast<const DWORD*>(default_vs),
            bd->vertex_shader.GetAddressOf());
    }
    return true;
}
...
void ImGui_ImplDX9_InvalidateDeviceObjects()
{
    ...
    bd->vertex_shader.Reset();
    bd->vertex_declaration.Reset();
}
