...
// You may use this file to debug, understand or extend Dear ImGui features but we don't provide any guarantee of
...
#pragma warning(disable : 26812) // The enum type 'xxx' is unscoped. Prefer 'enum class' over 'enum' (Enum.3). [MSVC
                                 // Static Analyzer)
#pragma warning(disable : 26495) // [Static Analyzer] Variable 'XXX' is uninitialized. Always initialize a member
                                 // variable (type.6).
...
#pragma clang diagnostic ignored "-Wfloat-equal" // warning: comparing floating point with == or != is unsafe // storing
                                                 // and comparing against same constants ok, for ImFloor()
...
#pragma clang diagnostic ignored                                                                                       \
    "-Wmissing-noreturn" // warning: function 'xxx' could be declared with attribute 'noreturn'
...
struct ImGuiInputTextDeactivateData; // Short term storage to backup text of a deactivating InputText() while another is
                                     // stealing active id
...
// Flags
typedef int ImGuiActivateFlags; // -> enum ImGuiActivateFlags_      // Flags: for navigation/focus function (will be for
                                // ActivateItem() later)
typedef int ImGuiDebugLogFlags; // -> enum ImGuiDebugLogFlags_      // Flags: for ShowDebugLogWindow(), g.DebugLogFlags
typedef int ImGuiFocusRequestFlags; // -> enum ImGuiFocusRequestFlags_  // Flags: for FocusWindow();
typedef int ImGuiInputFlags;        // -> enum ImGuiInputFlags_         // Flags: for IsKeyPressed(), IsMouseClicked(),
                                    // SetKeyOwner(), SetItemKeyOwner() etc.
...
// (since IMGUI_VERSION_NUM >= 18729: IMGUI_DEBUG_LOG was reworked into IMGUI_DEBUG_PRINTF (and removed framecount from
// it). If you were using a #define IMGUI_DEBUG_LOG please rename)
...
#define IM_NEWLINE "\n"
#endif
#ifndef IM_TABSIZE // Until we move this to runtime and/or add proper tab support, at least allow users to compile-time
                   // override
#define IM_TABSIZE (4)
#endif
...
// This will call IM_DEBUG_BREAK() which you may redefine yourself. See https://github.com/scottt/debugbreak for more
// reference.
...
static inline ImGuiID ImHashStr(const char *data, size_t data_size = 0, ImGuiID seed = 0)
{
    return ImHashStr(ImStrv(data, data_size ? data + data_size : NULL), seed);
}

// Helpers: Sorting
#ifndef ImQsort
static inline void ImQsort(void *base, size_t count, size_t size_of_element,
                           int(IMGUI_CDECL *compare_func)(void const *, void const *))
{
    if (count > 1)
...
IMGUI_API int ImStrnicmp(const char *str1, const char *str2,
                         size_t count); // Case insensitive compare to a certain count.
IMGUI_API void ImStrncpy(char *dst, ImStrv src,
                         size_t count); // Copy to a certain count and always zero terminate (strncpy doesn't).
IMGUI_API void ImStrncpy(char *dst, const char *src, size_t count);
IMGUI_API char *ImStrdup(ImStrv str); // Duplicate a string.
IMGUI_API char *ImStrdup(const char *str);
IMGUI_API char *ImStrdupcpy(char *dst, size_t *p_dst_size,
                            ImStrv str); // Copy in provided buffer, recreate buffer if needed.
...
IMGUI_API const ImWchar *ImStrbolW(const ImWchar *buf_mid_line,
                                   const ImWchar *buf_begin); // Find beginning-of-line (ImWchar string)
IM_MSVC_RUNTIME_CHECKS_OFF
static inline int ImStrcmp(const char *str1, const char *str2)
{
...
IMGUI_API int ImTextStrToUtf8(char *out_buf, int out_buf_size, const ImWchar *in_text,
...
IMGUI_API int ImTextStrFromUtf8(ImWchar *out_buf, int out_buf_size, const char *in_text, const char *in_text_end,
...
IMGUI_API const char *ImTextFindPreviousUtf8Codepoint(const char *in_text_start,
...
static inline ImFileHandle ImFileOpen(ImStrv, ImStrv)
{
    return NULL;
}
...
static inline float ImLength(const ImVec2 &lhs, float fail_value)
{
...
IMGUI_API ImVec2 ImBezierCubicClosestPoint(const ImVec2 &p1, const ImVec2 &p2, const ImVec2 &p3, const ImVec2 &p4,
                                           const ImVec2 &p,
...
#define IM_BITARRAY_CLEARBIT(_ARRAY, _N)                                                                               \
    ((_ARRAY[(_N) >> 5] &=                                                                                             \
      ~((ImU32)1 << ((_N) &                                                                                            \
                     31)))) // Macro version of ImBitArrayClearBit(): ensure args have side-effect or are costly!
...
template <int BITCOUNT, int OFFSET = 0> struct ImBitArray
{
...
{
    ImVector<ImDrawList *>
        *Layers[2]; // Pointers to global layers for: regular, tooltip. LayersP[0] is owned by DrawData.
...
enum ImGuiItemFlags_
{
    ...
    ImGuiItemFlags_NoNav = 1 << 3, // false     // Disable any form of focusing (keyboard/gamepad directional navigation
    ...
    ImGuiItemFlags_AllowOverlap = 1 << 9, // false     // Allow being overlapped by another widget. Not-hovered to
                                          // Hovered transition deferred by a frame.

    // Controlled by widget code
    ImGuiItemFlags_Inputable = 1 << 10, // false     // [WIP] Auto-activate input mode when tab focused. Currently only
                                        // used and supported by a few items before it becomes a generic feature.
...
enum ImGuiItemStatusFlags_
{
    ...
    ImGuiItemStatusFlags_Visible =
        1 << 9, // [WIP] Set when item is overlapping the current clipping rectangle (Used internally. Please don't use
                // yet: API/system will change as we refactor Itemadd()).
...
enum ImGuiHoveredFlagsPrivate_
{
    ...
    ImGuiHoveredFlags_AllowedMaskForIsWindowHovered =
        ...
        ImGuiHoveredFlags_NoPopupHierarchy | ImGuiHoveredFlags_AllowWhenBlockedByPopup |
        ImGuiHoveredFlags_AllowWhenBlockedByActiveItem | ImGuiHoveredFlags_ForTooltip | ImGuiHoveredFlags_Stationary,
...
enum ImGuiButtonFlagsPrivate_
{
    ...
    ImGuiButtonFlags_PressedOnClickRelease =
        1
        << 5, // [Default] return true on click + release on same item <-- this is what the majority of Button are using
    ...
    ImGuiButtonFlags_NoNavFocus =
        1 << 18, // don't override navigation focus when activated (FIXME: this is essentially used everytime an item
                 // uses ImGuiItemFlags_NoNav, but because legacy specs don't requires LastItemData to be set
                 // ButtonBehavior(), we can't poll g.LastItemData.InFlags)
    ImGuiButtonFlags_NoHoveredOnFocus = 1 << 19, // don't report as hovered when nav focus is on this item
    ImGuiButtonFlags_NoSetKeyOwner = 1 << 20, // don't set key/input owner on the initial click (note: mouse buttons are
                                              // keys! often, the key in question will be ImGuiKey_MouseLeft!)
    ImGuiButtonFlags_NoTestKeyOwner = 1 << 21, // don't test key/input owner when polling the key (note: mouse buttons
                                               // are keys! often, the key in question will be ImGuiKey_MouseLeft!)
...
enum ImGuiSelectableFlagsPrivate_
{
    ...
    ImGuiSelectableFlags_SelectOnNav =
        1 << 21, // (WIP) Auto-select when moved into. This is not exposed in public API as to handle multi-select and
                 ...
                 // BeginSelection() API.
    ImGuiSelectableFlags_SelectOnClick = 1
                                         << 22, // Override button behavior to react on Click (default is Click+Release)
...
struct ImGuiNextItemData
{
    ...
    ImGuiItemFlags ItemFlags; // Currently only tested/used for ImGuiItemFlags_AllowOverlap and
                              // ImGuiItemFlags_HasSelectionUserData.
    // Non-flags members are NOT cleared by ItemAdd() meaning they are still valid during NavProcessItem()
    float Width;          // Set by SetNextItemWidth()
    ImGuiID FocusScopeId; // Set by SetNextItemSelectionUserData()
    ImGuiSelectionUserData
        SelectionUserData; // Set by SetNextItemSelectionUserData() (note that NULL/0 is a valid value, we use -1 ==
...
// This is the minimum amount of data that we need to perform the equivalent of NavApplyItemToResult() and which we
// can't infer in TreePop() Only stored when the node is a potential candidate for landing on a Left arrow jump.
...
struct ImGuiInputEvent
{
    ...
    ImU32 EventId; // Unique, sequential increasing integer to identify an event (if you need to correlate them to other
                   // data).
    union {
...
// Input function taking an 'ImGuiID owner_id' argument defaults to (ImGuiKeyOwner_Any == 0) aka don't test ownership,
...
struct ImGuiKeyRoutingData
{
    ...
    ImU16 Mods; // Technically we'd only need 4-bits but for simplify we store ImGuiMod_ values which need 16-bits.
                // ImGuiMod_Shortcut is already translated to Ctrl/Super.
...
// Routing table: maintain a desired owner for each possible key-chord (key + mods), and setup owner in NewFrame() when
// mods are matching. Stored in main context (1 instance)
...
enum ImGuiInputFlags_
{
    ...
    ImGuiInputFlags_Repeat = 1 << 0, // Return true on successive repeats. Default for legacy IsKeyPressed(). NOT
                                     // Default for legacy IsMouseClicked(). MUST BE == 1.
    ...
    ImGuiInputFlags_LockThisFrame =
        1 << 6, // Access to key data will require EXPLICIT owner ID (ImGuiKeyOwner_Any/0 will NOT accepted for
                // polling). Cleared at end of frame. This is useful to make input-owner-aware code steal keys from
                // non-input-owner-aware code.
    ImGuiInputFlags_LockUntilRelease =
        1 << 7, // Access to key data will require EXPLICIT owner ID (ImGuiKeyOwner_Any/0 will NOT accepted for
                // polling). Cleared when the key is released or at end of each frame if key is released. This is useful
                // to make input-owner-aware code steal keys from non-input-owner-aware code.
    ...
    // - Priorities: GlobalHigh > Focused (when owner is active item) > Global > Focused (when focused window) >
    // GlobalLow.
    // - Can select only 1 policy among all available.
    ImGuiInputFlags_RouteFocused =
        1 << 8, // (Default) Register focused route: Accept inputs if window is in focus stack. Deep-most focused window
                // takes inputs. ActiveId takes inputs over deep-most focused window.
    ImGuiInputFlags_RouteGlobalLow = 1 << 9, // Register route globally (lowest priority: unless a focused window or
                                             // active item registered the route) -> recommended Global priority.
    ImGuiInputFlags_RouteGlobal = 1 << 10, // Register route globally (medium priority: unless an active item registered
                                           // the route, e.g. CTRL+A registered by InputText).
    ImGuiInputFlags_RouteGlobalHigh = 1 << 11, // Register route globally (highest priority: unlikely you need to use
                                               // that: will interfere with every active items)
    ImGuiInputFlags_RouteMask_ = ImGuiInputFlags_RouteFocused | ImGuiInputFlags_RouteGlobal |
                                 ImGuiInputFlags_RouteGlobalLow |
                                 ImGuiInputFlags_RouteGlobalHigh, // _Always not part of this!
    ImGuiInputFlags_RouteAlways = 1 << 12,                        // Do not register route, poll keys directly.
    ImGuiInputFlags_RouteUnlessBgFocused =
        1 << 13, // Global routes will not be applied if underlying background/void is focused (== no Dear ImGui windows
    ...
    ImGuiInputFlags_SupportedByShortcut = ImGuiInputFlags_Repeat | ImGuiInputFlags_RepeatRateMask_ |
                                          ImGuiInputFlags_RouteMask_ | ImGuiInputFlags_RouteExtraMask_,
...
enum ImGuiActivateFlags_
{
    ...
    ImGuiActivateFlags_PreferTweak = 1 << 1, // Favor activation for tweaking with arrows or gamepad (e.g. for
...
enum ImGuiScrollFlags_
{
    ...
    ImGuiScrollFlags_KeepVisibleEdgeX = 1 << 0, // If item is not visible: scroll as little as possible on X axis to
                                                // bring item back into view [default for X axis]
    ImGuiScrollFlags_KeepVisibleEdgeY =
        1 << 1, // If item is not visible: scroll as little as possible on Y axis to bring item back into view [default
                // for Y axis for windows that are already visible]
    ...
    ImGuiScrollFlags_NoScrollParent = 1 << 6, // Disable forwarding scrolling to parent window if required to keep
                                              // item/rect visible (only scroll window the function was applied to).
...
enum ImGuiNavMoveFlags_
{
    ...
    ImGuiNavMoveFlags_ScrollToEdgeY = 1 << 6, // Force scrolling to min/max (used by Home/End) // FIXME-NAV: Aim to
                                              // remove or reword, probably unnecessary
    ImGuiNavMoveFlags_Forwarded = 1 << 7,
    ImGuiNavMoveFlags_DebugNoResult = 1 << 8, // Dummy scoring for debug purpose, don't apply result
    ImGuiNavMoveFlags_FocusApi = 1 << 9,      // Requests from focus API can land/focus/activate items even if they are
                                         // marked with _NoTabStop (see NavProcessItemForTabbingRequest() for details)
...
enum ImGuiTypingSelectFlags_
{
    ...
    ImGuiTypingSelectFlags_AllowBackspace =
        1 << 0, // Backspace to delete character inputs. If using: ensure GetTypingSelectRequest() is not called more
                // than once per frame (filter by e.g. focus state)
...
struct IMGUI_API ImGuiTypingSelectRequest
{
    ...
    int SearchBufferLen;
    const char *SearchBuffer; // Search buffer contents (use full string. unless SingleCharMode is set, in which case
                              // use SingleCharSize).
    bool SelectRequest;       // Set when buffer was modified this frame, requesting a selection.
    bool SingleCharMode;      // Notify when buffer contains same character repeated, to implement special mode. In this
                              // situation it preferred to not display any on-screen search indication.
    ImS8 SingleCharSize;      // Length in bytes of first letter codepoint (1 for ascii, 2-4 for UTF-8). If
                              // (SearchBufferLen==RepeatCharSize) only 1 letter has been input.
};
...
struct IMGUI_API ImGuiTypingSelectState
{
    ...
    bool SingleCharModeLock =
        false; // After a certain single char repeat count we lock into SingleCharMode. Two benefits: 1) buffer never
               // fill, 2) we can provide an immediate SingleChar mode without timer elapsing.
...
struct IMGUI_API ImGuiMultiSelectTempData
{
    ...
    ImVec2 BackupCursorMaxPos;
    ImGuiID BoxSelectId;
    ImRect BoxSelectRectCurr; // Selection rectangle in absolute coordinates (derived from Storage->BoxSelectStartPosRel
                              // + MousePos)
    ...
    bool IsFocused;  // Set if currently focusing the selection scope (any item of the selection). May be used if you
                     // have custom shortcut associated to selection.
    bool IsSetRange; // Set by BeginMultiSelect() when using Shift+Navigation. Because scrolling may be affected we
...
struct IMGUI_API ImGuiMultiSelectState
{
    ...
    void Init(ImGuiID id)
    {
        ...
        BoxSelectActive = BoxSelectStarting = BoxSelectFromVoid = false;
        BoxSelectKeyMods = 0;
    }
};
...
struct ImGuiViewportP : public ImGuiViewport
{
    ...
    ImDrawList
        *BgFgDrawLists[2]; // Convenience background (0) and foreground (1) draw lists. We use them to draw software
...
// This is experimental and not officially supported, it'll probably fall short of features, if/when it does we may
// backtrack.
...
struct ImGuiStackLevelInfo
{
    ...
    char Desc[57]; // Arbitrarily sized buffer to hold a result (FIXME: could replace Results[] with a chunk stream?)
                   // FIXME: Now that we added CTRL+C this should be fixed.
...
struct ImGuiContext
{
    ...
    ImVector<ImGuiInputEvent> InputEventsTrail; // Past input events processed in NewFrame(). This is to allow
                                                // domain-specific application to access e.g mouse/pen trail.
    ...
    ImGuiWindow *MovingWindow; // Track the window we clicked on (in order to preserve focus). The actual window that is
                               // moved is generally MovingWindow->RootWindow.
    ...
    ImGuiID DebugHookIdInfo; // Will call core hooks: DebugHookIdInfo() from GetID functions, used by ID Stack Tool
    ...
    // - Input query can then read input by specifying ImGuiKeyOwner_Any (== 0), ImGuiKeyOwner_None (== -1) or a custom
    // ID.
    ...
    bool ActiveIdUsingAllKeyboardKeys; // Active widget will want to read all keyboard keys inputs. (FIXME: This is a
                                       // shortcut for not taking ownership of 100+ keys but perhaps best to not have
                                       // the inconsistency)
#ifndef IMGUI_DISABLE_OBSOLETE_KEYIO
    ImU32 ActiveIdUsingNavInputMask; // If you used this. Since (IMGUI_VERSION_NUM >= 18804) :
                                     // 'g.ActiveIdUsingNavInputMask |= (1 << ImGuiNavInput_Cancel);' becomes
                                     // 'SetKeyOwner(ImGuiKey_Escape, g.ActiveId) and/or
                                     // SetKeyOwner(ImGuiKey_NavGamepadCancel, g.ActiveId);'
#endif
    ...
    ImVector<ImGuiID> FocusScopeStack; // Stack for PushFocusScope()/PopFocusScope() - inherited by BeginChild(), pushed
                                       // into by Begin()
    ...
    ImGuiID
        NavActivateId; // ~~ (g.ActiveId == 0) && (IsKeyPressed(ImGuiKey_Space) || IsKeyDown(ImGuiKey_Enter) ||
                       // IsKeyPressed(ImGuiKey_NavGamepadActivate)) ? NavId : 0, also set when calling ActivateItem()
    ...
    ImGuiID
        NavJustMovedFromFocusScopeId; // Just navigated from this focus scope id (result of a successfully MoveRequest).
    ...
    bool NavDisableHighlight; // When user starts using mouse, we hide gamepad/keyboard highlight (NB: but they are
    ...
    ImGuiNavItemData NavInitResult; // Init request result (first item of the window, or one for which
    ...
    ImRect NavScoringNoClipRect; // Some nav operations (such as PageUp/PageDown) enforce a region which clipper will
                                 // attempt to always keep submitted
    int NavScoringDebugCount;    // Metrics for debugging
    int NavTabbingDir;           // Generally -1 or +1, 0 when tabbing without a nav id
    int NavTabbingCounter;       // >0 when counting items for tabbing
    ImGuiNavItemData NavMoveResultLocal;        // Best move request candidate within NavWindow
    ImGuiNavItemData NavMoveResultLocalVisible; // Best move request candidate within NavWindow that are mostly visible
                                                // (when using ImGuiNavMoveFlags_AlsoScoreVisibleSet flag)
    ...
    int MultiSelectTempDataStacked; // Temporary multi-select data size (because we leave previous instances
                                    // undestructed, we generally don't use MultiSelectTempData.Size)
    ...
    ImU8 DebugLocateFrames; // For DebugLocateItemOnHover(). This is used together with DebugLocateId which is in a
    ...
    int WantCaptureMouseNextFrame; // Explicit capture override via
                                   // SetNextFrameWantCaptureMouse()/SetNextFrameWantCaptureKeyboard(). Default to -1.
    ...
    ImGuiContext(ImFontAtlas *shared_font_atlas)
    {
        ...
        HoverItemDelayId = HoverItemDelayIdPreviousFrame = HoverItemUnlockedStationaryId =
...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    ImVec2 CursorMaxPos; // Used to implicitly calculate ContentSize at the beginning of next frame, for scrolling range
                         // and auto-resize. Always growing during the frame.
    ImVec2 IdealMaxPos; // Used to implicitly calculate ContentSizeIdeal at the beginning of next frame, for auto-resize
    ...
    ImVec1 GroupOffset;
    ImVec2 CursorStartPosLossyness; // Record the loss of precision of CursorStartPos due to really large scrolling
    ...
    bool NavIsScrollPushableX; // Set when current work location may be scrolled horizontally when moving left / right.
                               // This is generally always true UNLESS within a column.
...
struct IMGUI_API ImGuiWindow
{
    ...
    float DecoOuterSizeX1,
        DecoOuterSizeY1; // Left/Up offsets. Sum of non-scrolling outer decorations (X1 generally == 0.0f. Y1 generally
                         // = TitleBarHeight + MenuBarHeight). Locked during Begin().
    float DecoOuterSizeX2,
        DecoOuterSizeY2; // Right/Down offsets (X2 generally == ScrollbarSize.x, Y2 == ScrollbarSizes.y).
    float DecoInnerSizeX1, DecoInnerSizeY1; // Applied AFTER/OVER InnerRect. Specialized for Tables as they use
    ...
    ImGuiWindow *ParentWindow; // If we are a child _or_ popup _or_ docked window, this is pointing to our parent.
                               // Otherwise NULL.
    ImGuiWindow *ParentWindowInBeginStack;
    ImGuiWindow *RootWindow; // Point to ourself or first ancestor that is not a child window. Doesn't cross through
                             // popups/dock nodes.
    ImGuiWindow *RootWindowPopupTree; // Point to ourself or first ancestor that is not a child window. Cross through
                                      // popups parent<>child.
    ...
    ImVec2 NavPreferredScoringPosRel[ImGuiNavLayer_COUNT]; // Preferred X/Y position updated when moving on a given
  ...
  public:
    ...
    {
        return GetID(ImStrv(str));
    }
    ImGuiID GetID(const char *str, const char *str_end)
    {
        return GetID(ImStrv(str, str_end));
    }
...
struct ImGuiTabItem
{
    ...
    ImS16 IndexDuringLayout; // Index only used during TabBarLayout(). Tabs gets reordered so 'Tabs[n].IndexDuringLayout
...
struct ImGuiTableColumn
{
    ...
    float
        WidthGiven; // Final/actual width visible == (MaxX - MinX), locked in TableUpdateLayout(). May be > WidthRequest
                    // to honor minimum width, may be < WidthRequest to honor shrinking columns down in tight space.
    float MinX;     // Absolute positions
    float MaxX;
    float WidthRequest; // Master width absolute value when !(Flags & _WidthStretch). When Stretch this is derived every
...
// Per-instance data that needs preserving across frames (seemingly most others do not need to be preserved aside from
...
struct IMGUI_API ImGuiTable
{
    ...
    ImSpan<ImGuiTableColumnIdx> DisplayOrderToIndex; // Point within RawData[]. Store display order of columns (when not
    ...
    ImBitArrayPtr EnabledMaskByIndex; // Column Index -> IsEnabled map (== not hidden by user/api) in a format adequate
                                      // for iterating column without touching cold data
    ImBitArrayPtr VisibleMaskByIndex; // Column Index -> IsVisibleX|IsVisibleY map (== not hidden by user/api && not
                                      // hidden by scrolling/cliprect)
    ImGuiTableFlags SettingsLoadedFlags; // Which data were loaded from the .ini file (e.g. when order is not altered we
                                         // won't save order)
    ...
    int CurrentRow;
    int CurrentColumn;
    ImS16 InstanceCurrent; // Count of BeginTable() calls with same ID in the same frame (generally 0). This is a little
                           // bit similar to BeginCount for a window, but multiple table with same ID look are multiple
                           // tables, they are just synched.
    ...
    int RowBgColorCounter; // Counter for alternating background colors (can be fast-forwarded by e.g clipper), not same
                           // as CurrentRow because header rows typically don't increase this.
    ...
    ImRect BgClipRect; // We use this to cpu-clip cell background color fill, evolve during the frame as we cross frozen
                       // rows boundaries
    ...
    bool DisableDefaultContextMenu; // Disable default context menu contents. You may submit your own using
...
struct ImGuiTableSettings
{
    ...
    ImGuiTableColumnIdx ColumnsCountMax; // Maximum number of columns this settings instance can store, we can recycle a
                                         // settings instance with lower number of columns but not higher
    bool WantApply; // Set when loaded from .ini data (to enable merging/loading .ini data into an already running
                    // context)
...
IMGUI_API void Shutdown(); // Since 1.60 this is a _private_ function. You can call DestroyContext() to destroy the
...
IMGUI_API void SetLastItemData(ImGuiID item_id, ImGuiItemFlags in_flags, ImGuiItemStatusFlags status_flags,
...
{
    LogRenderedText(ref_pos, ImStrv(text, text_end));
}
IMGUI_API void LogSetNextTextDecoration(const char *prefix, const char *suffix);

// Popups, Modals, Tooltips
IMGUI_API bool BeginChildEx(ImStrv name, ImGuiID id, const ImVec2 &size_arg, ImGuiChildFlags child_flags,
...
// Menus
IMGUI_API bool BeginViewportSideBar(ImStrv name, ImGuiViewport *viewport, ImGuiDir dir, float size,
                                    ImGuiWindowFlags window_flags);
IMGUI_API bool BeginMenuEx(ImStrv label, ImStrv icon, bool enabled = true);
IMGUI_API bool MenuItemEx(ImStrv label, ImStrv icon, ImStrv shortcut = ImStrv(), bool selected = false,
                          bool enabled = true);
...
inline bool IsNamedKeyOrModKey(ImGuiKey key)
{
...
// - Ownership is most often claimed as a result of reacting to a press/down event (but occasionally may be claimed
// ahead).
// - Input queries can then read input by specifying ImGuiKeyOwner_Any (== 0), ImGuiKeyOwner_None (== -1) or a custom
// ID.
// - Legacy input queries (without specifying an owner or _Any or _None) are equivalent to using ImGuiKeyOwner_Any (==
// 0).
// - Input ownership is automatically released on the frame after a key is released. Therefore:
//   - for ownership registration happening as a result of a down/press event, the SetKeyOwner() call may be done once
//   (common case).
//   - for ownership registration happening ahead of a down/press event, the SetKeyOwner() call needs to be made every
//   frame (happens if e.g. claiming ownership on hover).
// - SetItemKeyOwner() is a shortcut for common simple case. A custom widget will probably want to call SetKeyOwner()
// multiple times directly based on its interaction state.
// - This is marked experimental because not all widgets are fully honoring the Set/Test idioms. We will need to move
// forward step by step.
//   Please open a GitHub Issue to submit your usage scenario or if there's a use case you need solved.
IMGUI_API ImGuiID GetKeyOwner(ImGuiKey key);
IMGUI_API void SetKeyOwner(ImGuiKey key, ImGuiID owner_id, ImGuiInputFlags flags = 0);
IMGUI_API void SetKeyOwnersForKeyChord(ImGuiKeyChord key, ImGuiID owner_id, ImGuiInputFlags flags = 0);
IMGUI_API void SetItemKeyOwner(
    ImGuiKey key,
    ImGuiInputFlags flags = 0); // Set key owner to last item if it is hovered or active. Equivalent to 'if
                                // (IsItemHovered() || IsItemActive()) { SetKeyOwner(key, GetItemID());'.
IMGUI_API bool TestKeyOwner(ImGuiKey key,
                            ImGuiID owner_id); // Test that key is either not owned, either owned by 'owner_id'
...
// - Important: legacy IsKeyPressed(ImGuiKey, bool repeat=true) _DEFAULTS_ to repeat, new IsKeyPressed() requires
// _EXPLICIT_ ImGuiInputFlags_Repeat flag.
// - Expected to be later promoted to public API, the prototypes are designed to replace existing ones (since owner_id
// can default to Any == 0)
// - Specifying a value for 'ImGuiID owner' will test that EITHER the key is NOT owned (UNLESS locked), EITHER the key
// is owned by 'owner'.
//   Legacy functions use ImGuiKeyOwner_Any meaning that they typically ignore ownership, unless a call to SetKeyOwner()
//   explicitly used ImGuiInputFlags_LockThisFrame or ImGuiInputFlags_LockUntilRelease.
// - Binding generators may want to ignore those for now, or suffix them with Ex() until we decide if this gets moved
// into public API.
IMGUI_API bool IsKeyDown(ImGuiKey key, ImGuiID owner_id);
IMGUI_API bool IsKeyPressed(ImGuiKey key, ImGuiID owner_id,
                            ImGuiInputFlags flags = 0); // Important: when transitioning from old to new IsKeyPressed():
                                                        // old API has "bool repeat = true", so would default to repeat.
                                                        // New API requiress explicit ImGuiInputFlags_Repeat.
...
// - When using one of the routing flags (e.g. ImGuiInputFlags_RouteFocused): routes requested ahead of time given a
// chord (key + modifiers) and a routing policy.
// - Routes are resolved during NewFrame(): if keyboard modifiers are matching current ones: SetKeyOwner() is called +
// route is granted for the frame.
// - Route is granted to a single owner. When multiple requests are made we have policies to select the winning route.
// - Multiple read sites may use the same owner id and will all get the granted route.
// - For routing: when owner_id is 0 we use the current Focus Scope ID as a default owner in order to identify our
// location.
// - TL;DR;
//   - IsKeyChordPressed() compares mods + call IsKeyPressed() -> function has no side-effect.
//   - Shortcut() submits a route then if currently can be routed calls IsKeyChordPressed() -> function has (desirable)
//   side-effects.
...
//   If you imagine an hypothetical BeginSelectionGroup()/EndSelectionGroup() api, it would likely call
...
IMGUI_API int TypingSelectFindMatch(ImGuiTypingSelectRequest *req, int items_count,
                                    const char *(*get_item_name_func)(void *, int), void *user_data, int nav_item_idx);
IMGUI_API int TypingSelectFindNextSingleCharMatch(ImGuiTypingSelectRequest *req, int items_count,
                                                  const char *(*get_item_name_func)(void *, int), void *user_data,
                                                  int nav_item_idx);
IMGUI_API int TypingSelectFindBestLeadingMatch(ImGuiTypingSelectRequest *req, int items_count,
                                               const char *(*get_item_name_func)(void *, int), void *user_data);
...
IMGUI_API void BeginColumns(ImStrv str_id, int count,
                            ImGuiOldColumnFlags flags = 0); // setup number of columns. use an identifier to distinguish
...
IMGUI_API int TableGetHoveredRow();    // Retrieve *PREVIOUS FRAME* hovered row. This difference with
                                       // TableGetHoveredColumn() is the reason why this is not public yet.
...
IMGUI_API bool BeginTableEx(ImStrv name, ImGuiID id, int columns_count, ImGuiTableFlags flags = 0,
...
IMGUI_API void TabItemLabelAndCloseButton(ImDrawList *draw_list, const ImRect &bb, ImGuiTabItemFlags flags,
                                          ImVec2 frame_padding, ImStrv label, ImGuiID tab_id, ImGuiID close_button_id,
...
inline void RenderText(ImVec2 pos, const char *text, const char *text_end, bool hide_text_after_hash = true)
{
    RenderText(pos, ImStrv(text, text_end), hide_text_after_hash);
}
...
{
    RenderTextWrapped(pos, ImStrv(text, text_end), wrap_width);
}
IMGUI_API void RenderTextClipped(const ImVec2 &pos_min, const ImVec2 &pos_max, ImStrv text,
...
inline void RenderTextClipped(const ImVec2 &pos_min, const ImVec2 &pos_max, const char *text, const char *text_end,
                              ...
                              const ImRect *clip_rect = NULL)
{
    RenderTextClipped(pos_min, pos_max, ImStrv(text, text_end), text_size_if_known, align, clip_rect);
}
IMGUI_API void RenderTextClippedEx(ImDrawList *draw_list, const ImVec2 &pos_min, const ImVec2 &pos_max, ImStrv text,
                                   const ImVec2 *text_size_if_known, const ImVec2 &align = ImVec2(0, 0),
                                   const ImRect *clip_rect = NULL);
...
{
    RenderTextClippedEx(draw_list, pos_min, pos_max, ImStrv(text, text_end), text_size_if_known, align, clip_rect);
}
IMGUI_API void RenderTextEllipsis(ImDrawList *draw_list, const ImVec2 &pos_min, const ImVec2 &pos_max, float clip_max_x,
                                  float ellipsis_max_x, ImStrv text, const ImVec2 *text_size_if_known);
...
{
    return FindRenderedTextEnd(ImStrv(text, text_end));
}
IMGUI_API void RenderMouseCursor(ImVec2 pos, float scale, ImGuiMouseCursor mouse_cursor, ImU32 col_fill,
...
IMGUI_API bool RenderWindowResizeGrip(ImDrawList *draw_list, const ImVec2 &corner, unsigned int rad,
                                      unsigned int overall_grip_size, ImDrawFlags flags, ImU32 col);
...
IMGUI_API void RenderRectFilledWithHole(ImDrawList *draw_list, const ImRect &outer, const ImRect &inner, ImU32 col,
                                        float rounding);

// Widgets
IMGUI_API void TextEx(ImStrv text, ImGuiTextFlags flags = 0);
inline void TextEx(const char *text, const char *text_end, ImGuiTextFlags flags = 0)
{
    TextEx(ImStrv(text, text_end), flags);
}
...
IMGUI_API bool ImageButtonEx(ImGuiID id, ImTextureID texture_id, const ImVec2 &image_size, const ImVec2 &uv0,
...
IMGUI_API bool ScrollbarEx(const ImRect &bb, ImGuiID id, ImGuiAxis axis, ImS64 *p_scroll_v, ImS64 avail_v,
...
IMGUI_API bool TreeNodeUpdateNextOpen(
    ImGuiID id,
    ImGuiTreeNodeFlags
        flags); // Return open state. Consume previous SetNextItemOpen() data, if any. May return true when logging.
...
IMGUI_API float ScaleRatioFromValueT(ImGuiDataType data_type, T v, T v_min, T v_max, bool is_logarithmic,
                                     float logarithmic_zero_epsilon, float zero_deadzone_size);
template <typename T, typename SIGNED_T, typename FLOAT_T>
IMGUI_API T ScaleValueFromRatioT(ImGuiDataType data_type, float t, T v_min, T v_max, bool is_logarithmic,
                                 float logarithmic_zero_epsilon, float zero_deadzone_size);
template <typename T, typename SIGNED_T, typename FLOAT_T>
...
// InputText
IMGUI_API bool InputTextEx(ImStrv label, ImStrv hint, char *buf, int buf_size, const ImVec2 &size_arg,
                           ImGuiInputTextFlags flags, ImGuiInputTextCallback callback = NULL, void *user_data = NULL);
IMGUI_API void InputTextDeactivateHook(ImGuiID id);
IMGUI_API bool TempInputText(const ImRect &bb, ImGuiID id, ImStrv label, char *buf, int buf_size,
                             ImGuiInputTextFlags flags);
IMGUI_API bool TempInputScalar(const ImRect &bb, ImGuiID id, ImStrv label, ImGuiDataType data_type, void *p_data,
...
// Plot
IMGUI_API int PlotEx(ImGuiPlotType plot_type, ImStrv label, float (*values_getter)(void *data, int idx), void *data,
                     int values_count, int values_offset, ImStrv overlay_text, float scale_min, float scale_max,
...
IMGUI_API void ShadeVertsTransformPos(ImDrawList *draw_list, int vert_start_idx, int vert_end_idx,
...
IMGUI_API void DebugAllocHook(ImGuiDebugAllocInfo *info, int frame_count, void *ptr,
...
IMGUI_API void DebugLocateItemOnHover(
...
IMGUI_API void DebugNodeDrawList(ImGuiWindow *window, ImGuiViewportP *viewport, const ImDrawList *draw_list,
                                 const char *label);
IMGUI_API void DebugNodeDrawCmdShowMeshAndBoundingBox(ImDrawList *out_draw_list, const ImDrawList *draw_list,
...
IMGUI_API void DebugNodeWindowsListByBeginStackParent(ImGuiWindow **windows, int windows_size,
...
// Refactored focus/nav/tabbing system in 1.82 and 1.84. If you have old/custom copy-and-pasted widgets that used
// FocusableItemRegister():
//  (Old) IMGUI_VERSION_NUM  < 18209: using 'ItemAdd(....)'                              and 'bool tab_focused =
//  FocusableItemRegister(...)' (Old) IMGUI_VERSION_NUM >= 18209: using 'ItemAdd(..., ImGuiItemAddFlags_Focusable)'  and
//  'bool tab_focused = (GetItemStatusFlags() & ImGuiItemStatusFlags_Focused) != 0' (New) IMGUI_VERSION_NUM >= 18413:
//  using 'ItemAdd(..., ImGuiItemFlags_Inputable)'     and 'bool tab_focused = (GetItemStatusFlags() &
//  ImGuiItemStatusFlags_FocusedTabbing) != 0 || (g.NavActivateId == id && (g.NavActivateFlags &
//  ImGuiActivateFlags_PreferInput))' (WIP)
...
IMGUI_API void ImFontAtlasBuildRender8bppRectFromString(ImFontAtlas *atlas, int x, int y, int w, int h,
                                                        const char *in_str, char in_marker_char,
                                                        unsigned char in_marker_pixel_value);
IMGUI_API void ImFontAtlasBuildRender32bppRectFromString(ImFontAtlas *atlas, int x, int y, int w, int h,
                                                         const char *in_str, char in_marker_char,
...
// Note that stroke width increases effective radius, so (e.g.) a max radius circle will have to use the fallback path
// if stroke width is > 1. Note that ImFontAtlasRoundCornersSizeMask is 64 bits so this value can only go up to a
// maximum of 64 at present.
const int ImFontAtlasRoundCornersMaxSize = 32; // Maximum size of rounded corner texture to generate in fonts
// Bit mask for which radii will have texture generated for them, starting from radius 1. Only bits up to
// ImFontAtlasRoundCornersMaxSize are considered.
const ImU64 ImFontAtlasRoundCornersSizeMask = (1ULL << ImFontAtlasRoundCornersMaxSize) - 1;
const int ImFontAtlasRoundCornersMaxStrokeWidth =
    4; // Maximum stroke width of rounded corner texture to generate in fonts
// Bit mask for which stroke widths should have textures generated for them (the default of 0x0B means widths 1, 2 and
// 4) Only bits up to ImFontAtlasRoundCornersMaxStrokeWidth are considered, and bit 0 (stroke width 1) must always be
// set Optimally there should be an odd number of bits set, as the texture packing packs the data in pairs, with one
// half of one pair being occupied by the filled texture
